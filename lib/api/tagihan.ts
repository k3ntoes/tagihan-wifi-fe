...
export const tagihanApi = {
    ...
    getAll: async (params: TagihanQueryParams): Promise<PaginationResponse<Tagihan>> => {
        const response = await apiClient.get<PaginationResponse<Tagihan>>('/tagihan', { params });
    ...
    // Update tagihan
    update: async (id: string, data: TagihanRequest): Promise<ApiSuccessMessage> => {
        const response = await apiClient.put<ApiSuccessMessage>(`/tagihan/${id}`, data);
        return response.data;
    },

    // Delete tagihan
    delete: async (id: string): Promise<ApiSuccessMessage> => {
        const response = await apiClient.delete<ApiSuccessMessage>(`/tagihan/${id}`);
        return response.data;
    },

    // Get summary by year
    getSummary: async (tahun: number): Promise<TagihanSummaryResponse> => {
        const response = await apiClient.get<TagihanSummaryResponse>(`/tagihan/summary/${tahun}`);
        return response.data;
    },
};
