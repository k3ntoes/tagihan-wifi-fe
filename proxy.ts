...
const protectedRoutes = ['/dashboard', '/paket', '/pelanggan', '/tagihan', '/users'];
...
export async function proxy(request: NextRequest) {
    ...
    if (!isValidSession && refreshToken) {
        try {
            ...
            if (response.ok) {
                ...
                const expires = payload.exp ? new Date(payload.exp * 1000) : new Date(Date.now() + 7 * 24 * 60 * 60 * 1000);
                ...
                if (newRefreshToken) {
                    ...
                    const refreshExpires = refreshPayload.exp ? new Date(refreshPayload.exp * 1000) : new Date(Date.now() + 7 * 24 * 60 * 60 * 1000);
    ...
    if (isPublicRoute && isValidSession && !request.nextUrl.pathname.startsWith('/dashboard')) {
        return NextResponse.redirect(new URL('/', request.nextUrl));
    }
